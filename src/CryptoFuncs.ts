...
export interface EncryptFuncs {
    encrypt(publicKey: Uint8Array, message: Uint8Array): { message: Uint8Array, nonce: Uint8Array }
}
...
export class Encryption extends types.IdentityEncryption {
    ...
    generateWithMasterPublicKey(publicKey: Uint8Array, salt: Uint8Array, creator: Encryption) {
    ...
    private generateMasterKey() {
        if (this.secret != null) {
            ...
            let masterKey = Crypto.PBKDF2(this.secret, this.masterSalt, 5000, nacl.secretbox.keyLength)
    ...
    private decryptKeyWithMasterKey(
        ...
        let cipher = {
            nonce, sign: creator, message: encryptedKey
        }
        return {
            publicKey,
...
class DecryptAnonymous implements DecryptFuncs {
    ...
    static decryptList(secretKey: Uint8Array, ciphers: ResolvedCipher[]): Uint8Array {
        return ciphers.reduce((secretKey, cipher) => DecryptAnonymous.decrypt(secretKey, cipher), secretKey)
    }
    decryptList(ciphers: ResolvedCipher[]): Uint8Array {
...
class DecryptSES implements DecryptFuncs {
    ...
    static decrypt(secretKey, { message, nonce, sign }: ResolvedCipher): Uint8Array {
    ...
    static decryptList(secretKey: Uint8Array, ciphers: ResolvedCipher[]): Uint8Array {
        return ciphers.reduce((secretKey, cipher) => DecryptSES.decrypt(secretKey, cipher), secretKey)
    }
    decryptList(ciphers: ResolvedCipher[]): Uint8Array {
