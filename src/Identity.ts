...
export class IdentityImpl implements IdentityAPI {
    ...
    return await this.session.doProtoRequest<void>({
      ...
      request: () =>
        api.IdentityCreateRequest.encode({
          ...
          encryption,
          email: options.email,
          signChain: this.session.encryption.sign(
            Uint8Tool.concat(
  ...
  async renewKeys(login: string, secret?: string | Uint8Array): Promise<void> {
    ...
    let { message, nonce } = this.session.encryption
  ...
  private async getSharingGraph(
    ...
    let firstSharingKey = (await this.session.getAssumeParams({
      login,
      kind: IdentityAccessKind.WRITE
    })).key.sharingKey;
    let x = resolvedGraph.map((elt, i) => {
      ...
      if (i == 0) {
        ...
        sharingKey = this.session.encryption
          .decrypt(api.ResourceType.SES, boxkey)
          .decrypt(elt.sharingKey);
      }
      let boxKey = this.session.encryption
        .decrypt(api.ResourceType.SES, sharingKey)
        .decrypt(elt.boxKey);
      let signKey = this.session.encryption
        .decrypt(api.ResourceType.SES, sharingKey)
        .decrypt(elt.signKey);
