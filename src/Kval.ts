...
export class Kval implements KvalAPI {
    ...
    async put(namespace: string, key: Uint8Array, value: Uint8Array): Promise<void> {
        let r = {
            namespace, key, value,
    ...
    async get(namespace: string, key: Uint8Array): Promise<{ value: Uint8Array; pk: IdentityPublicKeyID }> {
    ...
    private static messageToSign(
        _: { key?: Uint8Array; value?: Uint8Array; namespace?: string; created?: Long | number },
    ): Uint8Array {
        const ns = new TextEncoder().encode(_.namespace);
        let b = new Uint8Array(_.key.byteLength + _.value.byteLength + ns.byteLength + 8);
        let offset = 0;
        b.set(_.key, offset);
...
export class KvalDelegates implements KvalDelegatesAPI {
    ...
    public async get(login: string, application: string): Promise<string[]> {
        let r = await this.kval.get(KvalDelegates.NS, KvalDelegates.key(login, application));
