...
describe("delegatedAccess", () => {
  ...
  it("bob create a selfish resource", async () => {
    ...
    res = await new ResourceAPI(bobSession).create("test/A", payload, [
      bob.login
    ]);
  ...
  let accessRequest: DataPeps.DelegatedAccess.Request;
  ...
  let bobResolver: DataPeps.DelegatedAccess.RequestResolver;
  it("bob open the access request", async () => {
    bobResolver = await new DelegatedAccessAPI(bobSession).resolveAccessRequest(
      accessRequest.id
    );
  ...
  it("alice use the session sent by bob to get the resource", async () => {
    let resource = await new ResourceAPI(aliceBobSession).get(res.id);
