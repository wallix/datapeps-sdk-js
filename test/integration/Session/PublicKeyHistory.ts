...
describe('Session.getPublicKeyHistory', () => {
    ...
    it('Alice can get her public key', async () => {
        ...
        let aliceKeys = await aliceSession.Identity.getPublicKeyHistory(alice.login)
    ...
    it('Public key history contains old and new versions after admin overwrite key', async () => {
        ...
        await adminSession.Admin.overwriteKeys(alice.login, secret)
        aliceSession = await DataPeps.login(alice.login, secret)
