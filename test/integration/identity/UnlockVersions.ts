...
async function expectInaccessibleResource(session: DataPeps.Session, id: DataPeps.ID) {
    try {
        await session.Resource.get<TestResource>(id)
    } catch (e) {
        expect(e).to.exist
        expect(e.message).to.be.equal('DataPepsError(IdentityNotFound)')
        return
    }
...
describe('identity.UnlockVersions', () => {
    ...
    it('Create identity and resource v1', async () => {
        resv1 = await aliceSession.Resource.create("test", { description: "This is a test resource for Alice v1" }, [alice.login])
    })
    it('Create identity and resource v2', async () => {
        adminSession = await Config.adminLogin()
        await aliceSession.renewKeys() // v2
        aliceSession = await DataPeps.login(alice.login, aliceSecret)
        resv2 = await aliceSession.Resource.create("test", { description: "This is a test resource for Alice v2" }, [alice.login])
    })
    it('Create identity and resource v3', async () => {
        ...
        resv3 = await aliceSession.Resource.create("test", { description: "This is a test resource for Alice v3" }, [alice.login])
    })
    it('Create identity and resource v4', async () => {
        ...
        resv4 = await aliceSession.Resource.create("test", { description: "This is a test resource for Alice v4" }, [alice.login])
    })
    it('Create identity and resource v5', async () => {
        await aliceSession.renewKeys() // v5
        aliceSession = await DataPeps.login(alice.login, adminSecret1)
        resv5 = await aliceSession.Resource.create("test", { description: "This is a test resource for Alice v5" }, [alice.login])
    })
    it('Create identity and resource v6', async () => {
        ...
        resv6 = await aliceSession.Resource.create("test", { description: "This is a test resource for Alice v6" }, [alice.login])
    })
    it('Create identity and resource v7', async () => {
        await aliceSession.renewKeys() // v7
        aliceSession = await DataPeps.login(alice.login, adminSecret2)
        resv7 = await aliceSession.Resource.create("test", { description: "This is a test resource for Alice v7" }, [alice.login])
    })

    it('Resources created by locked versions are not accessible', async () => {
        expect(await aliceSession.Resource.get<TestResource>(resv6.id)).to.be.deep.equal(resv6);
        expect(await aliceSession.Resource.get<TestResource>(resv7.id)).to.be.deep.equal(resv7);
    ...
    it('Unlock no version when providing a bad secret', async () => {
        let unlockedSessions = await aliceSession.Identity.unlockVersions(alice.login, 'a bad secret');
        expect(unlockedSessions.length).to.be.equal(0);
    })


    it('Unlock several versions which shares the same secret', async () => {
        let unlockedSessions = await aliceSession.Identity.unlockVersions(alice.login, adminSecret1);
        expect(unlockedSessions.length).to.be.equal(2);
        expect(unlockedSessions.map(unlocked => unlocked.version).sort((a, b) => a - b)).to.be.deep.equal([4, 5])
    })

    it('After unlock of v4 and v5, the related resources become accessible', async () => {
        expect(await aliceSession.Resource.get<TestResource>(resv4.id)).to.be.deep.equal(resv4);
        expect(await aliceSession.Resource.get<TestResource>(resv5.id)).to.be.deep.equal(resv5);
        await expectInaccessibleResource(aliceSession, resv1.id);
        await expectInaccessibleResource(aliceSession, resv2.id);
        await expectInaccessibleResource(aliceSession, resv3.id);
    })
    ...
    it('Unlock v1 and v2', async () => {
        let unlockedSessions = await aliceSession.Identity.unlockVersions(alice.login, aliceSecret);
        expect(unlockedSessions.length).to.be.equal(2);
        expect(unlockedSessions.map(unlocked => unlocked.version).sort((a, b) => a - b)).to.be.deep.equal([1, 2])
    })

    it('Unlock v3', async () => {
        let unlockedSessions = await aliceSession.Identity.unlockVersions(alice.login, otherPassword);
        expect(unlockedSessions.length).to.be.equal(1);
        expect(unlockedSessions.map(unlocked => unlocked.version)).to.be.deep.equal([3])
    })

    it('After unlock all versions are accessible', async () => {
        expect(await aliceSession.Resource.get<TestResource>(resv1.id)).to.be.deep.equal(resv1);
        expect(await aliceSession.Resource.get<TestResource>(resv2.id)).to.be.deep.equal(resv2);
        expect(await aliceSession.Resource.get<TestResource>(resv3.id)).to.be.deep.equal(resv3);
        expect(await aliceSession.Resource.get<TestResource>(resv4.id)).to.be.deep.equal(resv4);
        expect(await aliceSession.Resource.get<TestResource>(resv5.id)).to.be.deep.equal(resv5);
        expect(await aliceSession.Resource.get<TestResource>(resv6.id)).to.be.deep.equal(resv6);
        expect(await aliceSession.Resource.get<TestResource>(resv7.id)).to.be.deep.equal(resv7);
    ...
    it('Create new locked versions for tests from a device (with assume)', async () => {
        ...
        let unlockedSessions = await deviceSession.Identity.unlockVersions(alice.login, adminSecret2)
        expect(unlockedSessions.length).to.be.equal(2);
        expect(unlockedSessions.map(unlocked => unlocked.version).sort((a, b) => a - b)).to.be.deep.equal([6, 7])
        expect(unlockedSessions.map(unlocked => unlocked.login)).to.be.deep.equal([alice.login, alice.login])
